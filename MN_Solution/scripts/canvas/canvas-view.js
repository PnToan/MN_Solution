...
window.MN_drawDemoObjects = function(width, height) {
  ...
  state.demoObjects.forEach(function(obj) {
    ...
    ctx.restore();
    if (obj.type === 'panel_box' && typeof window.MN_drawPanelOffsetDim === 'function') {
      window.MN_drawPanelOffsetDim(obj, currentView);
    }
    if (obj.dimEnabled) {
      drawDim(
        x,
...
window.MN_openDimInlineEditor = function(area) {
  ...
  if (area.key === 'panel_offset_dim') {
    ...
    const obj = objects.find(function(item) {
      return item.id === area.panelId;
    });

    if (!obj || obj.type !== 'panel_box') return;

    currentValue = Number(obj.panelOffset || 0);
    allowZero = true;

    commitTarget = function(newValue) {
      if (typeof window.MN_applyPanelOffset === 'function') {
        window.MN_applyPanelOffset(obj, newValue);
      }
    };
  } else if (area.key === 'demo_object_dim') {
    const objects = window.MN_AppState && Array.isArray(window.MN_AppState.demoObjects)
      ? window.MN_AppState.demoObjects
      : [];
